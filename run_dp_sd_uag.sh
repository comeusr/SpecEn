...
# Script to run speculative decoding with UAG (Using Assistant Generation) in data parallel setting
# Usage: ./run_dp_sd_uag.sh [num_gpus]
...
DIFFERENT_TOKENIZER="True"  # Set different tokenizer to True
...
for TARGET in "${!MODEL_PAIRS[@]}"; do
    ...
    for NUM_DRAFT in "${DRAFT_LENGTHS[@]}"; do
        ...
        # Set model path
        MODEL_PATH="../data/${DATASET}/model/${TARGET}_${DRAFT}_sd_uag_${DIFFERENT_TOKENIZER}_13Aug_ksr_NUM_EXMPLS${N_EXAMPLES}_TEMP${TEMPERATURE}_DO_SAMPLe${DO_SAMPLE}/"
        
        # Create the output directory if it doesn't exist
        mkdir -p $MODEL_PATH
        
        # Run the data parallel version using torchrun with UAG script
        torchrun --nproc_per_node=$NUM_GPUS \
            $(dirname "$0")/speculative_decoding_dp_uag.py \
            ...
            --different_tokenizer $DIFFERENT_TOKENIZER \
            > .logs/${DATASET}/${TARGET}_${DRAFT}_${METHOD}_UAG${DIFFERENT_TOKENIZER}_${STATIC_EW}_13Aug_DRAFT_LEN${NUM_DRAFT}_${ITER}_NUM_EXMPLS${N_EXAMPLES}_TEMP${TEMPERATURE}_DO_SAMPLe${DO_SAMPLE}.log 2>&1
        
        echo "Completed ${DATASET} with target: ${TARGET}, draft: ${DRAFT}, draft length: ${NUM_DRAFT}"
    done
done

echo "All experiments completed for WMT dataset with UAG in data parallel setting!"
