...
def first_false_index(bool_tensor):
    batch, k = bool_tensor.shape
    false_mask = ~bool_tensor
    has_false = false_mask.any(dim=1, keepdim=True)
    idx = false_mask.float().argmax(dim=1, keepdim=True)
    return torch.where(has_false, idx, torch.full_like(idx, k))


def target_distribution_diff(
    probs_small: torch.Tensor,
    probs_large: torch.Tensor,
    lenience: float = -1.0
) -> torch.Tensor:
    """
    Target distribution for Diff deferral rule (PyTorch version).
    
    Args:
        probs_small: [batch_size, vocab_size], softmaxed small model output
        probs_large: [batch_size, vocab_size], softmaxed large model output
        lenience: float, how much less confident the small model can be

    Returns:
        Combined distribution based on the diff rule
    """
    max_prob_small, _ = probs_small.max(dim=-1, keepdim=True)
    max_prob_large, _ = probs_large.max(dim=-1, keepdim=True)

    # Criterion: max_v p_small(v) >= max_v p_large(v) - lenience
    pick_small = (max_prob_small >= (max_prob_large - lenience)).float()  # shape [B, 1]

    return pick_small * probs_small + (1.0 - pick_small) * probs_large
...
def target_distribution_opt(
    ...
    """Target distribution for OPT deferral rule in PyTorch."""

    # max_v p_small(v) and p_large(v)
    max_prob_small = torch.max(probs_small, dim=-1, keepdim=True).values
    max_prob_large = torch.max(probs_large, dim=-1, keepdim=True).values

    # TVD: sum over vocab dimension of max(0, p_small - p_large)
    tvd = torch.sum(torch.clamp(probs_small - probs_large, min=0.0), dim=-1, keepdim=True)

    # OPT criterion
    pick_small = (max_prob_small >= (max_prob_large - lenience * tvd)).float()  # shape: [B, T, 1]
...
def target_distribution_token_v1(
    ...
    """Target distribution for Token-v1 deferral rule in PyTorch."""
    # max_u p_large_unscaled(u) over vocab axis
    max_prob_large = torch.max(probs_small, dim=-1, keepdim=True).values  # shape: [B, T, 1]

    # Token-v1 criterion: p_small_unscaled(v) >= max_u p_large_unscaled(u) - lenience
    tokens_accepted = (probs_small >= (max_prob_large - lenience)).float()  # shape: [B, T, V]

    # Accepted small model probs
    probs_small_accepted = probs_small * tokens_accepted

    # Compute remaining mass and assign it to large model
    probs_small_accepted_sum = 1.0 - probs_small_accepted.sum(dim=-1, keepdim=True)  # shape: [B, T, 1]
...
def target_distribution_token_v3(
    ...
    """Target distribution for Token-v3 deferral rule in PyTorch."""
    # max_u p_large_unscaled(u) for each token position
    token_prob_large = torch.max(probs_large, dim=-1, keepdim=True).values  # shape: [B, T, 1]

    # Token-v3 criterion: p_large_unscaled(v) >= (1 - lenience) * max_u p_large_unscaled(u)
    tokens_accepted = (probs_large >= token_prob_large * (1 - lenience)).float()  # [B, T, V]

    # Accepted tokens from small model
    probs_small_accepted = probs_small * tokens_accepted

    # Remaining mass goes to large model
    probs_small_rejected_sum = 1.0 - probs_small_accepted.sum(dim=-1, keepdim=True)  # [B, T, 1]

    # Final distribution
    return probs_small_accepted + probs_small_rejected_sum * probs_large


def target_distribution_entropy(
    probs_small: torch.Tensor,
    probs_large: torch.Tensor,
    lenience: float = 0.0    
) -> torch.Tensor:

    # probs_small, probs_large: [1, 5, V]
    eps = 1e-8  # to avoid log(0)
    
    # 1. Compute entropy of probs_small: [1, 5]
    entropy_small = -torch.sum(probs_small * torch.log(probs_small + eps), dim=-1)
    entropy_large = -torch.sum(probs_large * torch.log(probs_large + eps), dim=-1)
    
    # 2. Compute inverse entropy: [1, 5]
    inv_entropy_small = 1.0 / (entropy_small + eps)
    inv_entropy_large = 1.0 / (entropy_large + eps)
    
    # 3. Compute weights with softmax over inverse entropy: [1, 5]
    inv_entropy = torch.cat([inv_entropy_small, inv_entropy_large], dim=-1)
    weights = F.softmax(inv_entropy, dim=-1)  # optional: dim=-1 (across sequence)
    
    # 4. Reshape weights for broadcasting: [1, 5, 1]
    w_small = weights[:, :, 0].unsqueeze(-1)
    w_large = weights[:, :, 1].unsqueeze(-1)
    
    # 5. Compute weighted sum: more confident (lower entropy) gets higher weight
    return w_small * probs_small + w_large * probs_large  # [1, 5, V]


def target_distribution_constant(
    probs_small: torch.Tensor,
    probs_large: torch.Tensor,
    lenience: float = 0.0    
) -> torch.Tensor:

    return (1-lenience)*probs_small+lenience*probs_large
...
class RejectionSampler(SpecDecodeStochasticBaseSampler):
    ...
    def _batch_modified_rejection_sampling(
        ...
        # Modify the target distirbution here
        CONFIG = config_holder.CONFIG
        ensemble_method = getattr(CONFIG.method, "ensemble", '')
        if ensemble_method == "token_v3":
            target_probs = target_distribution_token_v3(draft_probs, target_probs, CONFIG.method.alpha)
        elif ensemble_method == "constant":
            target_probs = target_distribution_constant(draft_probs, target_probs, CONFIG.method.alpha)
        elif ensemble_method == "entropy":
            target_probs = target_distribution_entropy(draft_probs, target_probs, CONFIG.method.alpha)
        elif ensemble_method == "diff":
            target_probs = target_distribution_diff(draft_probs, target_probs, CONFIG.method.alpha)
        else:
        ...
        # collect the result
        print("[Accepted Num Tokens]: ", first_false_index(accepted).sum().item(), "[Total Num Tokens]: ", accepted.numel())
