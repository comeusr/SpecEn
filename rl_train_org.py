...
def main(config: DictConfig):
    ...
    device_map = {
    "target_model": [0,1],  # Use first 4 GPUs for target model
    "draft_model": [0,1],         # Use next 2 GPUs for draft model
    }
    ...
    target_model = AutoModelForCausalLM.from_pretrained(config.model.target_name_or_path, 
                                                ...
                                                #   device_map=device_map["target_model"]
                                                  device_map="auto"
                                                  )
    ...
    draft_model = AutoModelForCausalLM.from_pretrained(config.model.draft_name_or_path, 
                                                  ...
                                                  trust_remote_code=True, 
                                                  attn_implementation="flash_attention_2",
                                                #   device_map=device_map["draft_model"]
                                                   device_map="auto"
                                                  )

    model = EnsembleWrapper(target_model, draft_model, True)
    model.ensemble_head = model.ensemble_head.cuda()
    model = model.to(dtype=torch.bfloat16) # added by kasasiva
