...
def main(args):
    try:
        ...
        # UAG: Load assistant model and tokenizer
        assistant_model = None
        assistant_tokenizer = None
        atm_translator = None

        if args.different_tokenizer == "True":
            assistant_tokenizer = AutoTokenizer.from_pretrained(args.draft_model, trust_remote_code=True)
        ...
        if 'gemma' in args.target_model:
            for m in (model, draft_model, assistant_model):
                if m is not None:
                ...
                with torch.no_grad():
                    ...
                    if assistant_tokenizer is not None:
                        ...
                        output_ids = model.generate(
                            ...
                            # atm_translator=atm_translator,  # Pass the translator
                        ...
                        # Original generation methods (sd, sd_en, static_en)
                        output_ids = model.generate(
                            input_ids,
                            attention_mask=attn_mask,
                            max_new_tokens=args.max_tokens,
...
if __name__ == "__main__":
    ...
    parser.add_argument('--different_tokenizer', type=str, choices=["True", "False"], help="Whether draft and target have different tokenziers.")
